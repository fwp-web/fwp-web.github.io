...
Tree.prototype.showTraverse = function(query) {
    ...
    removeClass(this.treeNodes[0], "default");
    addClass(this.treeNodes[0], "traverse");
    timer = setInterval(function() {
        i++;
        if(i < that.treeNodes.length) {
            if(that.treeNodes[i-1].firstChild.nodeValue.trim().toLowerCase() !== querystr) {
                removeClass(that.treeNodes[i-1], "traverse");
                addClass(that.treeNodes[i-1], "default");
            } else {
                removeClass(that.treeNodes[i-1], "traverse");
                addClass(that.treeNodes[i-1], "search");
                isfind = true;
            }
            removeClass(that.treeNodes[i], "default");
            addClass(that.treeNodes[i], "traverse");
        } else {
            if(that.treeNodes[i-1].firstChild.nodeValue.trim().toLowerCase() !== querystr) {
                removeClass(that.treeNodes[i-1], "traverse");
                addClass(that.treeNodes[i-1], "default");
